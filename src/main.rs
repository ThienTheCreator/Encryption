...
fn rijndael_key_setup_enc(rk: &mut [u32], cipher_key: &[u8], key_bits: u32) -> usize {
    ...
    if key_bits == 128 {
        loop {
            ...
            rk[7 + index] = rk[3 + index] ^ rk[6 + index];
            i += 1;
            if i == 10 {
                return 10;
            }
    ...
    rk[4 + index] = get_u32!(cipher_key[16..20]);
    rk[5 + index] = get_u32!(cipher_key[20..24]);
    if key_bits == 192 {
        loop {
            temp = rk[5 + index] as usize;
            rk[6 + index] = rk[0 + index]
                ^ (TE4[{ temp >> 16 } & 0xff])
                ^ (TE4[(temp >> 8) & 0xff] & 0x00ff0000)
                ^ (TE4[(temp) & 0xff] & 0x0000ff00)
                ^ (TE4[(temp >> 24)] & 0x000000ff)
                ^ RCON[i];
            rk[7 + index] = rk[1 + index] ^ rk[6 + index];
            rk[8 + index] = rk[2 + index] ^ rk[7 + index];
            rk[9 + index] = rk[3 + index] ^ rk[8 + index];
            i += 1;
            if i == 8 {
                return 12;
            }
            rk[10 + index] = rk[4 + index] ^ rk[9 + index];
            rk[11 + index] = rk[5 + index] ^ rk[10 + index];
            index += 6;
        }
    }

    rk[6 + index] = get_u32!(cipher_key[24..28]);
    rk[7 + index] = get_u32!(cipher_key[28..32]);
    if key_bits == 256 {
        loop {
            temp = rk[7 + index] as usize;
            rk[8] = rk[0]
                ...
                ^ (TE4[(temp >> 24)] & 0x000000ff)
                ^ RCON[i];
            rk[9 + index] = rk[1 + index] ^ rk[8 + index];
            rk[10 + index] = rk[2 + index] ^ rk[9 + index];
            rk[11 + index] = rk[3 + index] ^ rk[10 + index];
            i += 1;
            if i == 7 {
                return 14;
            }
            temp = rk[11 + index] as usize;
            rk[12 + index] = rk[4 + index]
                ^ (TE4[(temp >> 24)] & 0xff000000)
                ^ (TE4[(temp >> 16) & 0xff] & 0x00ff0000)
                ^ (TE4[(temp >> 8) & 0xff] & 0x0000ff00)
                ^ (TE4[(temp) & 0xff] & 0x000000ff);
            rk[13 + index] = rk[5 + index] ^ rk[12 + index];
            rk[14 + index] = rk[6 + index] ^ rk[13 + index];
            rk[15 + index] = rk[7 + index] ^ rk[14 + index];
...
fn main() -> io::Result<()> {
    ...
    let mut bytes_left = f.metadata().unwrap().len();
